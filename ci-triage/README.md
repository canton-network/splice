...
10143 is the third failed job of run 35072334729 (the one not covered by 10141/10142).

| My ref | GH run | Branch / sha | Failed job | Mapping |
|--------|--------|--------------|------------|---------|
| 10135 | 35052914872 | release-line-0.8.1 66a5e3f02b (#7313 backport of #7310) | 104656956648 `static_tests` | inferred |
| 10136 | 35052864473 | release-line-0.8.0 7cafbf8ef1 (#7314 backport of #7310) | 104657010559 `wall-clock-time (8)` | inferred |
...
|--------|--------------|--------------------|----------|---------------------|--------------|
| 10135 | 35052914872 static_tests (release-line-0.8.1) | `apps-app / scalafixAll` dies with "Unable to load symbol table: .../classes/splice-wallet-0.1.22.dar.<uuid>.tmp": scalafix indexed the classes dir while `copyResources` was atomically staging a dar into it. The "ErrorResponse.scala contained different content" lines are guardrail noise present in passing runs too. | build flake (sbt task race) | Packet `10135-scalafix-dar-copy-race.md`. Not a regression: #7313/#7310 is pulumi/docs only and the same change passed static_tests on 0.8.0 and main. Re-run. Build fix: `scalafix dependsOn copyResources` (Compile and Test), or stop syncing 222 dars into `apps-app` classes. | none known |
| 10136 | 35052864473 wall-clock-time (8) (release-line-0.8.0, canton 3.5.16) | SvOnboardingIntegrationTest: `initDso()` times out after 5 min on sv3. SEQ::sv3 joins the BFT ordering topology (size = strongQuorum = 3) at epoch 21 while two epochs behind and still waiting for onboarding state; it receives all of epoch 20 but never enters 21; nothing is ordered again. sv3's app then spins on "No traffic state found for MED::sv3" from its own frozen sequencer. Teardown plugin `sys.exit(1)`s, so no ScalaTest report. | real bug, canton BFT onboarding | Packet `10136-sv3-init-timeout-bft-onboarding-wedge.md`. Same mechanism and log signatures as 10048 (2->3 membership step with the newcomer two epochs behind; the "named leader before init" detail in 10048 is incidental). New finding: sv3's SequencerSynchronizerState became effective 170 us after epoch 19's topology tick, which slipped its membership from epoch 20 to 21. Hand to Canton BFT team; splice mitigation options in packet. | 10048 family (BFT onboarding, 27+ CI occurrences) |
...
- `sys.exit(1)` in ResetTopologyStatePlugin (ResetDecentralizedNamespace) destroyed the evidence in both
  10136 and 10139: no ScalaTest summary, no junit XML, checkErrors skipped. Same pattern noted for the
...
- 10136 confirms the BFT onboarding wedge family is still live on release-line-0.8.0 (canton 3.5.16).
