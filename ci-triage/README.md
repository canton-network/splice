...
| 10088 | 34474728903 | (A) roll-forward-lsu: `InvalidStaticSynchronizerParameters` over non-default synchronizerLimits at SV init; (B) LSU: bobValidatorLocal hangs 5 min because sv4 published its sequencer successor too late (past upgrade time) | Evidence packet `10088-lsu-failures.md`. A = canton 3.6 static-param check rejects LSU predecessor params (jobs cancelled). B (splice-side): sv4 onboarded ~2 min late, finished LSU sequencer init ~43s past the 12:26:56 upgrade time -> `LsuNodeInitializer` skipped publishing sv4's successor -> `No sequencer successor was found` -> bob's migration hangs -> "Condition never became true within 5 minutes". |
