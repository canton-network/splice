...
trait SynchronizerFeesTestUtil extends TestCommon {
  ...
  /** Have a validator purchase traffic on behalf of a target member. */

  def buyMemberTrafficFor(
      payerApp: ValidatorAppBackendReference,
      targetMember: Member,
      trafficAmount: Long,
  )(implicit env: SpliceTestConsoleEnvironment): AmuletOperationOutcome = {
    val ts = env.environment.clock.now
    val payerParty = payerApp.getValidatorPartyId()

    val (synchronizerId, transferContext) =
      clue(s"Resolve active synchronizer and transfer context for $payerParty at $ts") {
        ...
        val ctx = sv1ScanBackend.getTransferContextWithInstances(ts)
        (syncId, ctx)
      }

    val walletInstall = clue(s"Fetch WalletAppInstall for $payerParty") {
      val installs = payerApp.participantClientWithAdminToken.ledger_api_extensions.acs
        .filterJava(WalletAppInstall.COMPANION)(
          payerParty,
          c => c.data.validatorParty == c.data.endUserParty,
        )
      inside(installs) { case Seq(install) => install }
    }

    val inputAmuletsDaml = clue(s"Fetch available amulets for coin selection for $payerParty") {
      val amulets = listValidatorContracts(splice.amulet.Amulet.COMPANION)(payerApp)
      amulets
        .map(a =>
          new splice.amuletrules.transferinput.InputAmulet(a.id): splice.amuletrules.TransferInput
        )
        .asJava
    }

    val executeBatchCmd = clue("Construct WalletAppInstall_ExecuteBatch command") {
      walletInstall.id.exerciseWalletAppInstall_ExecuteBatch(
    ...
    val outcomes =
      clue(s"Submit batch command to ledger as user ${payerApp.config.ledgerApiUser}") {
        payerApp.participantClientWithAdminToken.ledger_api_extensions.commands
          .submitWithResult(
            payerApp.config.ledgerApiUser,
            Seq(payerParty),
            Seq(payerParty),
            executeBatchCmd,
    ...
    clue(s"Extract AmuletOperationOutcome (expected 1, got ${outcomes.size})") {
      inside(outcomes) { case Seq(outcome) =>
        outcome
      }
    }
  }
