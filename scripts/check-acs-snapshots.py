...
parser.add_argument("--acs-snapshot-time", type=str, default="2099-01-01T00:00:00Z", help="Before time (ISO 8601 format, e.g., 2025-12-31T17:10:55Z)")
parser.add_argument("--acs-snapshot-limit", type=int, default=10000, help="Maximum number of updates to compare per snapshot")
parser.add_argument("--import-updates-limit", type=int, default=10000, help="Maximum number of import updates to compare")
parser.add_argument("--checks", type=str, default="migration_info,import_updates,acs_snapshots", help="Comma-separated list of checks to run (e.g., migration_info,import_updates,acs_snapshots)")
args = parser.parse_args()


max_concurrent_downloads = args.max_concurrent_downloads
num_examples = args.num_examples
acs_snapshot_limit = args.acs_snapshot_limit
acs_snapshot_time = args.acs_snapshot_time
import_updates_limit = args.import_updates_limit
...
print_section_header("Starting check with the following parameters")
console.print(f"Migration ID: [cyan]{migration_id}[/cyan]")
console.print(f"Trusted scan URL: [cyan]{trusted_scan}[/cyan]")
console.print(f"Checks: [cyan]{checks}[/cyan]")
console.print(f"ACS snapshot time: before [cyan]{acs_snapshot_time}[/cyan]")
console.print(f"ACS snapshot limit: [cyan]{acs_snapshot_limit}[/cyan]")
console.print(f"Import updates limit: [cyan]{import_updates_limit}[/cyan]")

print_section_header("Fetching list of all scans...")
...
if "migration_info" in checks:
    print_section_header("Checking migration info...")
    migration_infos={}
    for url in scan_urls:
        payload={
...
if "import_updates" in checks:
    print_section_header("Checking import updates...")
    console.print(f"Using the first {import_updates_limit} updates for comparison.")
            ...
            while True:
                ...
                if result.status_code == 200:
                    ...
                    if len(new_transactions_ids) > 0:
                        ...
                        if len(import_update_ids) >= import_updates_limit:
                            break
...
if "acs_snapshot" in checks:
    # Get latest snapshot time before `acs_snapshot_time` that is available on all scans
    print_section_header("Picking snapshot time...")
    snapshot_times={}
    for url in scan_urls:
        try:
            latest=requests.get(f"{url}/api/scan/v0/state/acs/snapshot-timestamp?before={acs_snapshot_time}&migration_id={migration_id}").json()
            if latest.get("record_time") is not None:
                snapshot_times[url]=latest["record_time"]
                console.print(f"{url}: ${latest["record_time"]}")
            else:
                console.print(f"{url}: no snapshots")
        except Exception as e:
            console.print(f"{url}: failed to get snapshot time")

    snapshot_time=min(snapshot_times.values())
    console.print(f"Using {snapshot_time}")

    # Compare snapshot
    print_section_header(f"Comparing snapshots...")
    console.print(f"Using the first {acs_snapshot_limit} contracts for comparison.")
        ...
        try:
            while True:
                ...
                if result.status_code == 200:
                    ...
                    if next_page_token is None or len(cids) >= import_updates_limit:
                        break
                else:
                    break
            progress.update(task_id, status=f"[green]Done")
    ...
    for url in [url for url in cids if url != trusted_scan]:
        if cids[url] is None:
            console.print(f"✖ {url} failed to fetch acs snapshot!")
            continue
        if cids[trusted_scan] is None:
            console.print(f"? {url}: trusted scan failed to fetch acs snapshot!")
            continue
